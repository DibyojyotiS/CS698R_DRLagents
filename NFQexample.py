...
from CS698R_DRLagents import NFQ
...
explorationStrategyTrain = decayWrapper(selectEpsilonGreedyAction, 0.8, 0.1, 500, device=device)
NFQagent = NFQ(Qnetwork, env, 0, 0.99, 10, 10000, 512, optim.Adam, 0.001, 800, 1, explorationStrategyTrain, selectGreedyAction, device=device)


# train the agent and evaluate
train_stats = NFQagent.trainAgent()
eval_rewards = NFQagent.evaluateAgent()
...
for i_episode in range(5):
    ...
    for t in range(600):
        env.render()
        action = selectGreedyAction(NFQagent.Qnetwork, torch.tensor([observation], dtype=torch.float32, device=device))
